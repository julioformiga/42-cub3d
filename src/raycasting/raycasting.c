...
void	raycasting(t_env *env)
{
	...
	{
		ray_angle = (env->map.player.direction - (angle / 2.0))
			+ ((angle / (double)(env->map.numrays - 1)) * i);
		while (ray_angle < 0)
			ray_angle += 2 * M_PI;
		...
		ft_mlx_draw_line(env, (t_point){env->map.player.x, env->map.player.y},
			(t_point){env->map.player.x + ray.dir_x * ray_length,
			env->map.player.y + ray.dir_y * ray_length}, RED);
	}
}
